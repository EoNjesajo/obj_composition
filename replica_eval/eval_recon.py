...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=False):
    ...
    # completion_ratio_rec *= 100  # convert to %s
    # precision_ratio_rec *= 100  # convert to %
    # fscore *= 100
    # normal_acc *= 100
    # normal_comp *= 100
    # normal_avg *= 100
    print('Acc:', round(accuracy_rec,4), ',Comp:', round(completion_rec,4), 'Champer-L1:', round((accuracy_rec+completion_rec)*0.5,4) ,',Precision:', round(precision_ratio_rec,4), ',Recalls:', round(completion_ratio_rec,4), ',F_score:', round(fscore,4))
    ...
    data_color = cv2.applyColorMap(im_gray, cv2.COLORMAP_BONE)[:,0,[2, 0, 1]] / 255. #COLORMAP_JET
    ...
    stl_color = cv2.applyColorMap(im_gray, cv2.COLORMAP_BONE)[:,0,[2, 0, 1]] / 255. #COLORMAP_JET
