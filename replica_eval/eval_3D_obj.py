...
def get_obj_ids_ours(obj_dir, max_epoch):
    ...
    max_epoch = max_epoch
    # print(max_epoch)
    obj_file = [os.path.basename(f).split('.')[0].split('_')[2] for f in files if os.path.basename(f).split('_')[1]==max_epoch]
...
def get_gt_mesh_from_objid(gt_dir, mapping_list):
    ...
    if len(mapping_list) == 1:
        return trimesh.load(os.path.join(gt_dir, 'mesh_semantic.ply_'+str(mapping_list[0])+'.ply'))
    else:
        for idx in mapping_list:
            if os.path.isfile(os.path.join(gt_dir, 'mesh_semantic.ply_'+str(int(idx))+'.ply')):
                obj_file = os.path.join(gt_dir, 'mesh_semantic.ply_'+str(int(idx))+'.ply')
                obj_mesh = trimesh.load(obj_file)
...
if __name__ == "__main__":
    ...
    data_dir = f"{info_dir}/vmap" # where to store the data
    log_dir = './evaluation_results' # where to store the evaluation results
    
    train_type = 'objsdfplus'
    max_epoch = '100'
    mesh_rec_root = f"result/{train_type}" # path to the reconstruction results
    ...
    for idx, exp in enumerate(exp_name):
        ...
        mesh_rec_dir = os.path.join(mesh_rec_root,f'scan{idx}')#sorted(os.listdir(os.path.join(mesh_rec_root,f'scan{idx}')))
        # print(mesh_rec_dir)
        ...
        obj_ids, max_epoch = get_obj_ids_ours(mesh_rec_dir, max_epoch)
        # print(obj_ids)
        for obj_id in tqdm(obj_ids):
            
            inst_id = label_mapping[int(obj_id)]
            ...
            mesh_gt = get_gt_mesh_from_objid(gt_dir, gt_inst_list)
            if obj_id == 0:
                N=200000
            else:
                N=10000
