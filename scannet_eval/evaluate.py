...
def evaluate(mesh_pred, mesh_trgt, threshold=.05, down_sample=.02):
    ...
    metrics = {
        'Acc': np.mean(dist2),
        'Comp': np.mean(dist1),
        'Chamfer-L1' : (np.mean(dist2)+np.mean(dist1))*0.5,
        'Prec': precision,
        'Recal': recal,
        'F-score': fscore,
    }
...
approach = 'ours'
root_dir = f"result/{approach}" # path to your experiments
exp_name = "scan"   # experiment name
out_dir =f"evaluation/{approach}/eval_mesh" # output directory for evaluation results
...
for idx, scan in enumerate(scenes):
    ...
    print(cur_root)
    files = list(filter(os.path.isfile, glob.glob(os.path.join(cur_root, "*_whole.ply")))) # the whole mesh path
    ...
    # save mesh
    out_mesh_path = os.path.join(out_dir, f"{approach}_scan_{idx}_{scan}.ply")
...
for scan, metric in zip(scenes, all_results):
    values = [scan] + [str(round(metric[k],4)) for k in metric.keys()]
    out = ",".join(values)
    print(out)
...
for key in all_results[0].keys():
    mean_dict[key] = round(sum(d[key] for d in all_results) / len(all_results),4)
print(mean_dict)
