...
class SDFusionMultiModal2ShapeModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        print("SDFusion isTrain :",self.isTrain)
        ...
        # init vqvae
        self.vqvae = load_vqvae(vq_conf, ckpt=opt.vq_ckpt, opt=opt)
    ...
    def forward(self):
        ...
        c_img = self.img_linear(c_img) * p_img[:, None, None] #p_img가 true면 그대로 사용, false일 경우에는 모두 0
        ...
        return z_noisy.clone()


    def train(self, data, ddim_steps=None, ddim_eta=0., uc_scale=None,  #ddim_step 10, ddim_eta 0.1, uc_scale 5
                  txt_scale=1.0, img_scale=0.0, mm_cls_free=False):
        ...
        if not mm_cls_free:
            ...
            samples, _= self.ddim_sampler.sample(S=ddim_steps,
                                                            batch_size=B,
                                                            shape=shape,
                                                            conditioning=c_mm,
                                                            verbose=False,
        ...
        return self.vqvae_module.decode_no_quant(samples)
    ...
    def optimize_parameters(self, total_steps):
        ...
        self.backward()
        self.optimizer.step()
        result = self.vqvae_module.decode_no_quant(result)
        return result
    ...
    # def save(self, label, save_opt=False):
    ...
    def load_ckpt(self, ckpt_path):
        ...
        print(colored('[*] weight successfully load from: %s' % ckpt_path, 'blue'))
