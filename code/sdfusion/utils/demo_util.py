...
class SDFusionOpt(BaseOpt):
    ...
    def init_model_args(
        ...
        self.model = 'sdfusion'
        self.df_cfg = 'sdfusion/configs/sdfusion_snet.yaml'
        self.ckpt = ckpt_path
...
class SDFusionText2ShapeOpt(BaseOpt):
    ...
    def init_model_args(
        ...
        self.df_cfg = 'sdfusion/configs/sdfusion-txt2shape.yaml'
        self.ckpt = f'{path}/{ckpt_name}.pth'
        
        self.vq_model = 'vqvae'
        self.vq_cfg = 'sdfusion/configs/vqvae_snet.yaml'
        self.vq_ckpt = f'{path}/vqvae-snet-all.pth'
        self.vq_dset = 'snet'
        self.vq_cat = 'all'
...
class SDFusionImage2ShapeOpt(BaseOpt):
    ...
    def init_model_args(
        ...
        self.df_cfg = 'sdfusion/configs/sdfusion-img2shape.yaml'
        self.ckpt = ckpt_path
...
class SDFusionMM2ShapeOpt(BaseOpt):
    ...
    def init_model_args(
            self, 
            path, 
            ckpt_name = 'sdfusion-mm2shape',
            lr = 0, 
            isTrain = True
        ):
        # self.model = ckpt_name
        self.lr = lr
        self.isTrain = isTrain
...
def preprocess_image(image, mask):
    ...
    return img_comp, img_clean, bbox
...
def get_shape_mask(mask_mode, device='cuda'):
    ...
    # # # z_mask[:, :, :, :, :] = 0
    # if mask_mode[0] != 0:
    #     z_mask[:, :, :int(mask_mode[0]*16), :, :] = 1
    #     z_mask[:, :, 16-int(mask_mode[0]*16):, :, :] = 1
    #     # print(mask_mode[0])
    #     # print(mask_mode[0]*16, int(mask_mode[0]*16))
    # if mask_mode[1] != 0:
    #     z_mask[:, :, :, :, :int(mask_mode[1]*16)] = 1
    #     z_mask[:, :, :, :, 16-int(mask_mode[1]*16):] = 1    
    #     # print(mask_mode[1])
    #     # print(mask_mode[1]*16, int(mask_mode[1]*16))
    # if mask_mode[2] != 0:
    #     z_mask[:, :, :, :int(mask_mode[2]*16), :] = 1
    #     z_mask[:, :, :, 16-int(mask_mode[2]*16):, :] = 1
    ...
    # if x != None :
    #     idx = torch.where(x<0)
    #     x_mask = torch.ones(B, 1, D, D, D).to(device)
    #     x_mask[idx] = 0
    #     z_mask = F.interpolate(x_mask, size=(16, 16, 16), mode='trilinear', align_corners=False)
    #     x_mask = x_mask.bool()
    # else : 
    #     x_mask = torch.ones(B, 1, D, D, D).bool().to(device)
    #     z_mask = torch.ones(B, 1, d, d, d).to(device)
