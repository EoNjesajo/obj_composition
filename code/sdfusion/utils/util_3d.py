...
def save_mesh_from_sdf(sdf, out_name, level=0.02, color=None, render_all=False):
    # device='cuda'
    # device=sdf.device
    ...
    for i in range(nimg_to_render):
        # sdf_i = sdf[i, 0].detach().cpu().numpy()
        # verts_i, faces_i = mcubes.marching_cubes(sdf_i, 0.02)
        verts_i, faces_i = mcubes.marching_cubes(sdf, level)
        verts_i = verts_i / n_cell - .5 
        rgb = np.ones_like(verts_i)
        if color is not None:
            for j in range(3):
                rgb[:, j] = color[j]

        meshexport = trimesh.Trimesh(verts_i, faces_i, vertex_colors=rgb)
        meshexport.export(f"{out_name}" + '.ply', 'ply')
