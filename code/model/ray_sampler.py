...
class UniformSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model):
        ...
        # t_vals = torch.linspace(-1., 1., steps=self.N_samples).cuda()
        # z_vals = t_vals[192:]
...
class ErrorBoundSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, N_samples_eval, N_samples_extra,
        ...
        super().__init__(near, 2.0 )
        self.N_samples = N_samples
        self.N_samples_eval = N_samples_eval
        self.uniform_sampler = UniformSampler(scene_bounding_sphere, near, N_samples_eval, take_sphere_intersection=False, far=2.0) # replica scannet and T&T courtroom
        ...
        if inverse_sphere_bg:
            self.inverse_sphere_sampler = UniformSampler(1.0, 0.0, N_samples_inverse_sphere, False, far=2.0)

    def get_z_vals(self, ray_dirs, cam_loc, model, grid_sdf=None, target=None):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                if target == None : 
                    ...
                    samples_sdf = model.implicit_network.get_sdf_vals(points_flat, grid_sdf, target)
