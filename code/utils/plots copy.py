...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary, level=0):
    ...
    if plot_data is not None:
        ...
        # # plot sem maps
        plot_sem_maps(plot_data['semantic_map'], plot_data['semantic_gt'], path, epoch, plot_nimgs, img_res, indices)
...
def get_surface_trace(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], level=0):
...
def get_semantic_surface_trace(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], level=0, num=0): # default resolution 256 -> 64
    grid = get_grid_uniform(resolution, grid_boundary)
    points = grid['grid_points']
