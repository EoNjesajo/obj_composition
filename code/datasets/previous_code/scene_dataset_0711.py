...
class SceneDatasetDN_segs(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ground_truth = {
            ...
            "instance_mask": self.semantic_images[idx]
        }
        ...
        if self.sampling_idx is not None:
            if (self.random_image_for_path is None) or (idx not in self.random_image_for_path):
                ...
                ground_truth["instance_mask"] = self.semantic_images[idx][self.sampling_idx, :]
                ...
                ground_truth["full_mask"] = self.mask_images[idx]
                ground_truth["instance_mask"] = self.semantic_images[idx][patch_sampling_idx, :]
...
class RICO_SceneDatasetDN_Mask(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for spath in semantic_paths:
            ...
            self.instance_masks.append(torch.from_numpy(semantic.reshape(-1, 1)).float())
