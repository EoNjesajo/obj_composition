...
class CompletionDataset(torch.utils.data.Dataset):
    ...
    def change_target_obj(self, object_id, object_class, pixel_threshold):
        self.object_id = object_id
        temp = self.temp[object_id]
        print(f"total data list({len(temp)}) : {temp}")
        ...
        for scene_idx in temp : 
            ...
            if bbox_pixel > pixel_threshold:
                ...
                data_list.append(scene_idx)


        while(len(self.img_cleans)!=5):
            max_value = max(value)
            max_idx = value.index(max_value)
            
            self.img_cleans.append(img_cleans[max_idx]) 
            self.data_list.append(data_list[max_idx])
            self.bboxs.append(bboxs[max_idx])
