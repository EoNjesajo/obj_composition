        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            if self.GPU_INDEX == 0 and self.do_vis and epoch % self.plot_freq == 0:
                ...
                plt.plot(self.model.module.implicit_network,
                        ...
                        epoch,
                        self.img_res,
                        self.model.module.rendering_network,
                        **self.plot_conf
                        )

                self.model.train()
            self.all_dataset.change_sampling_idx(self.num_pixels, sampling_pattern="patch")
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                if self.GPU_INDEX == 0 and data_index %20 == 0:
                    print(
                        '{0} [{1}/{2}] ({3}/{4}): loss = {5}, rgb_loss = {6}, eikonal_loss = {7}, psnr = {8}, bete={9}, alpha={10}'
                            .format(self.expname, epoch, self.nepochs, data_index, self.n_batches, loss.item(),
                ...
                # if self.GPU_INDEX == 0 and data_index %20 == 0:
                #     print(
                #         '{0} [{1}/{2}] ({3}/{4}): loss = {5}, rgb_loss = {6}, eikonal_loss = {7}, psnr = {8}, bete={9}, alpha={10}, semantic_loss = {11}, reg_loss = {12}'
                #             .format(self.expname, epoch, self.nepochs, data_index, self.n_batches, loss.item(),
                #                     loss_output['rgb_loss'].item(),
                #                     loss_output['eikonal_loss'].item(),
                #                     psnr.item(),
